...
- Experiment 2: Training Time Dependency

Member B: The Training Tuner (Experiments 2 & 3)
Experiment 2: Training Time Dependency
    Goal: Find the point of diminishing returns for training.
    Method: Use the corrected baseline model and learning_rate=1e-3.
    Test: Run experiments for total_time = [0.25, 0.5, 1.0, 2.0, 4.0] hours.
    Deliverable: Plots of Error_u, Error_v, Error_w, Error_p vs. total_time. This will show how the error converges.
"""
...
if __name__ == "__main__": 
    ...
    # Log file
    log_file_path = "../Results/Member_B_Time_results.txt"
    pathlib.Path("../Results/").mkdir(exist_ok=True)
    with open(log_file_path, 'w') as f:
        f.write("MEMBER B EXPERIMENT 2 (TIME) LOG\n")
        f.write("="*30 + "\n")
        f.write("Experiment, Error_c, Error_u, Error_v, Error_w, Error_p\n")

    # --- Experiment 2: Training Time Test ---
    print("\n" + "="*50)
    print("MEMBER B: STARTING EXPERIMENT 2 (TIME TEST)")
    print("="*50)
    
    time_tests = [0.25, 0.5, 1.0, 2.0, 4.0]

    for time_val in time_tests:
        config = BASE_CONFIG.copy()
        config['name'] = f"Exp2_Time_{time_val}h"
        config['total_time'] = time_val
        config['learning_rate'] = 1e-3 # Fixed LR for this test
        config['results_dir'] = f"../Results/Member_B/Time/{config['name']}" # Subfolder
    ...
    print("\n" + "="*50)
    print("MEMBER B: TIME EXPERIMENTS COMPLETE.")
