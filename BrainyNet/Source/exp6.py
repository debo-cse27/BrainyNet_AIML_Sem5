"""
Experiment 6: Lambda Variation (Physics vs Data)
Variations: 0.1, 0.3, 0.5, 0.7, 0.9, 0.99
Loss = lambda * physics + (1-lambda) * data
...
from utilities import neural_net, Navier_Stokes_3D, tf_session, mean_squared_error, relative_error
...
class HFM(object):
    def __init__(self, t_data, x_data, y_data, z_data, c_data,
        ...
        # Loss components
        self.loss_data = mean_squared_error(self.c_data_pred, self.c_data_tf)
        ...
        # Dynamic Lambda
        self.lambda_ph = tf.placeholder(tf.float32, shape=[])
        
        # loss = lambda * physics + (1-lambda) * data
        self.loss = self.lambda_ph * self.loss_physics + (1.0 - self.lambda_ph) * self.loss_data
    ...
    def train(self, total_time, learning_rate, lambda_val):
    ...
    def predict(self, t, x, y, z):
        tf_dict = {self.t_data_tf: t, self.x_data_tf: x, self.y_data_tf: y, self.z_data_tf: z}
        return self.sess.run([self.c_data_pred, self.u_data_pred, self.v_data_pred, self.w_data_pred, self.p_data_pred], tf_dict)

if __name__ == "__main__": 
    results_dir = "../Results/Experiment6_Lambda"
    ...
    t_data = T_star_grid[:, idx_t][idx_x,:].flatten()[:,None]
    x_data = X_star_grid[:, idx_t][idx_x,:].flatten()[:,None]
    y_data = Y_star_grid[:, idx_t][idx_x,:].flatten()[:,None]
    z_data = Z_star_grid[:, idx_t][idx_x,:].flatten()[:,None]
    ...
    for lam in lambdas:
        ...
        model.train(total_time=2.0, learning_rate=1e-3, lambda_val=lam)
        ...
        vtk_dir = f"{results_dir}/vtk_lambda_{lam}"
        ...
        scipy.io.savemat(f"{results_dir}/results_lambda_{lam}.mat",
