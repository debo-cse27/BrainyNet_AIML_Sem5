...
- Experiment 1: Loss Weighting (Lambda)
- Experiment 4: Data Density (N_data)

Experiment 1: Loss Weighting
    Goal: Find the best balance between data loss and physics loss.
    Method: Your user request loss = lambda * physics + (1-lambda) * data is a good start. 
        In your new model, this will be: self.loss = (1.0 - self.lambda) * (self.loss_data + self.loss_bc) + self.lambda * self.loss_physics 
        You will need to add self.lambda = tf.placeholder(tf.float32, shape=[]) and pass it to tf_dict.
    Test: Run experiments for lambda = [0.1, 0.3, 0.5, 0.7, 0.9, 0.99]. (A high lambda means trusting physics more).
    Deliverable: Plots of Error_u, Error_v, Error_w, Error_p vs. lambda.

Experiment 4: Boundary Condition & Data Study
    Goal: Prove the importance of the changes made in Phase 1.
    Method: This is an ablation study on the data itself.
    Run 1: Baseline (Interior Data + BC Data + Physics).
    Run 4: Vary N_data and N_bc (e.g., 1000, 5000, 10000 points) to see how data density affects error.
    Deliverable: A table comparing the final errors from Runs 1-3. Plots of error vs. N_data. 
        This will be a key part of your project report.
"""
...
class HFM(object):
    def __init__(self, t_data, x_data, y_data, z_data, c_data,
                       ...
                       layers, batch_size,
                       Pec, Rey,
                       activation=tf.nn.tanh): # <-- MODIFIED: Added activation
        ...
        # <-- MODIFIED: Pass activation to neural_net
    ...
    def train(self, total_time, learning_rate, lambda_physics=0.5, lr_schedule=None, n_iter_print=10):
        ...
        while running_time < total_time:
            ...
            # <-- MODIFIED: Added learning rate schedule logic
            current_lr = learning_rate
            if lr_schedule == 'exponential_decay':
                # simple exponential decay: 1e-3 -> 1e-5 over total_time
            ...
            tf_dict = {self.t_data_tf: t_data_batch,
                       ...
                       self.learning_rate: current_lr, # <-- MODIFIED
...
def run_experiment(config):
    ...
    # Data Sampling
    T_data = T
    N_data = config.get('N_data', N) # Use N if not specified
    N_eqns = config.get('N_eqns', N) # Use N if not specified
    
    if N_data > N: N_data = N
    if N_eqns > N: N_eqns = N
    
    print(f"Using N_data = {N_data}, N_eqns = {N_eqns}")
    ...
    model.train(total_time = config['total_time'], 
                ...
                n_iter_print = 500) # Print less often for long runs
...
# --- Main execution block for Member C ---
if __name__ == "__main__": 
    
    # Baseline settings for Member C's experiments
    BASE_CONFIG = {
    ...
    # Log file
    log_file_path = "../Results/Member_C_results.txt"
    pathlib.Path("../Results/").mkdir(exist_ok=True)
    with open(log_file_path, 'w') as f:
        f.write("MEMBER C EXPERIMENT LOG\n")
        f.write("="*30 + "\n")
        f.write("Experiment, Error_c, Error_u, Error_v, Error_w, Error_p\n")

    # --- Experiment 1: Lambda (Loss Weighting) Test ---
    print("\n" + "="*50)
    print("MEMBER C: STARTING EXPERIMENT 1 (LAMBDA TEST)")
    print("="*50)
    
    lambda_tests = [0.1, 0.3, 0.5, 0.7, 0.9, 0.99]

    for lambda_val in lambda_tests:
        config = BASE_CONFIG.copy()
        config['name'] = f"Exp1_Lambda_{lambda_val}"
        config['lambda_physics'] = lambda_val
        config['results_dir'] = f"../Results/Member_C/{config['name']}"
    ...
    # --- Experiment 4: N_data (Data Density) Test ---
    print("\n" + "="*50)
    print("MEMBER C: STARTING EXPERIMENT 4 (N_DATA TEST)")
    print("="*50)
    
    # Need to get total N from data first to set the upper bound
    data = scipy.io.loadmat(BASE_CONFIG['data_path'])
    N_total = data['x_star'].shape[0]
    print(f"Total number of spatial points (N_total) = {N_total}")

    # We vary both N_data and N_eqns together
    ndata_tests = [5000, 10000, 20000, 40000, N_total]

    for n_val in ndata_tests:
        if n_val > N_total:
            n_val = N_total
            
        config = BASE_CONFIG.copy()
        config['name'] = f"Exp4_Ndata_{n_val}"
        config['N_data'] = n_val
        config['N_eqns'] = n_val # Varying both together
        config['results_dir'] = f"../Results/Member_C/{config['name']}"
    ...
    print("\n" + "="*50)
    print("MEMBER C: ALL EXPERIMENTS COMPLETE.")
