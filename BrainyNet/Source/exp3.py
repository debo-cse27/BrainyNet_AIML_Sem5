"""
Experiment 3: Network Depth Variation
Variations: 5, 8, 10, 12 layers (Width = 250)
Fixed Training Time: 2 hours
Fix: Uses tf.Graph().as_default() to isolate runs.
"""
...
if __name__ == "__main__": 
    results_dir = "../Results/Experiment3_Depth"
    if not os.path.exists(results_dir): os.makedirs(results_dir)
    
    depth_variations = [5, 8, 10, 12]
    fixed_width = 250
    ...
    for depth in depth_variations:
        print(f"\n======== STARTING RUN: Depth = {depth} layers ========\n")
        
        # FIX: Use Graph context to isolate runs
        graph = tf.Graph()
        with graph.as_default():
            layers = [4] + depth * [fixed_width] + [5]
            ...
            vtk_dir = f"{results_dir}/vtk_depth_{depth}"
            ...
            scipy.io.savemat(f"{results_dir}/results_depth_{depth}.mat",
...
# """
# Experiment 3: Network Depth Variation
# Variations: 10, 12 layers (Width = 250) - (5 and 8 skipped as requested)
# Fixed Training Time: 2 hours
# Fix: Uses tf.Graph().as_default() to isolate runs.
# """
...
#     # Modified variations to run only 10 and 12
#     depth_variations = [10, 12]
#     fixed_width = 250
