...
- Experiment 5: Ablation Study on Architecture (Depth & Width)
- Experiment 6: Ablation Study on Activation Function

Experiment 5: Ablation Study on Architecture
    Goal: See how network depth and width affect accuracy.
    Method: Use the best learning rate and time from Member B.
    Depth Test: Fix width (e.g., 250 neurons = 5*50). Test [4] + 5*[250] + [5], [4] + 8*[250] + [5], [4] + 10*[250] + [5] (baseline), [4] + 12*[250] + [5].
    Width Test: Fix depth (e.g., 10 layers). Test [4] + 10*[100] + [5], [4] + 10*[250] + [5] (baseline), [4] + 10*[400] + [5].
    Deliverable: Plots of Error_u, Error_v, Error_w, Error_p vs. "Number of Layers" and "Number of Neurons".

Experiment 6: Hyperparameter Tuning
    Goal: Optimize other NN-specific parameters.
    Method: Use the best architecture from your first experiment.
    Activation Function: Test tf.nn.tanh (current) vs. tf.nn.swish (SiLU), which often works well for PINNs.
    Deliverable: A comparison table showing the final errors for each configuration.
"""
...
if __name__ == "__main__": 
    ...
    # Baseline settings for Member A's experiments
    # Assumes Member B found 1.0h and 1e-3 LR are good
    BASE_CONFIG = {
        'total_time': 1.0, 
        'learning_rate': 1e-3, 
        'lambda_physics': 0.5, 
    ...
    # Log file
    log_file_path = "../Results/Member_A_results.txt"
    pathlib.Path("../Results/").mkdir(exist_ok=True)
    with open(log_file_path, 'w') as f:
        f.write("MEMBER A EXPERIMENT LOG\n")
        f.write("="*30 + "\n")
        f.write("Experiment, Error_c, Error_u, Error_v, Error_w, Error_p\n")

    # --- Experiment 5: Depth Test ---
    print("\n" + "="*50)
    print("MEMBER A: STARTING EXPERIMENT 5 (DEPTH TEST)")
    print("="*50)
    
    depth_tests = {
        '5x250': [4] + 5*[5*50] + [5],
        '8x250': [4] + 8*[5*50] + [5],
        '10x250': [4] + 10*[5*50] + [5], # Baseline
        '12x250': [4] + 12*[5*50] + [5],
    }

    for name, layers in depth_tests.items():
        config = BASE_CONFIG.copy()
        config['name'] = f"Exp5_Depth_{name}"
    ...
    # --- Experiment 5: Width Test ---
    print("\n" + "="*50)
    print("MEMBER A: STARTING EXPERIMENT 5 (WIDTH TEST)")
    print("="*50)
    
    width_tests = {
        '10x100': [4] + 10*[100] + [5],
        '10x250': [4] + 10*[250] + [5], # Baseline
        '10x400': [4] + 10*[400] + [5],
    }

    for name, layers in width_tests.items():
        config = BASE_CONFIG.copy()
        config['name'] = f"Exp5_Width_{name}"
        config['layers'] = layers
        config['results_dir'] = f"../Results/Member_A/{config['name']}"
    ...
    # --- Experiment 6: Activation Function Test ---
    print("\n" + "="*50)
    print("MEMBER A: STARTING EXPERIMENT 6 (ACTIVATION TEST)")
    print("="*50)
    ...
    # Use best architecture from Exp 5 (assuming baseline 10x250)
    best_layers = [4] + 10*[5*50] + [5] 

    for name, act_fn in activation_tests.items():
        config = BASE_CONFIG.copy()
        config['name'] = f"Exp6_Activation_{name}"
        config['layers'] = best_layers
    ...
    print("\n" + "="*50)
    print("MEMBER A: ALL EXPERIMENTS COMPLETE.")
