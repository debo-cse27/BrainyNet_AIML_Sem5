...
import tensorflow.compat.v1 as tf  # <-- MODIFIED: Use v1 compat directly
tf.disable_v2_behavior()         # <-- MODIFIED: Call disable_v2_behavior
import numpy as np
import scipy.io
import time
import sys
...
class HFM(object):
    ...
    def __init__(self, t_data, x_data, y_data, z_data, c_data,
                       u_data, v_data, w_data, p_data,        # <-- MODIFIED: Added u, v, w, p data
                       t_eqns, x_eqns, y_eqns, z_eqns,
                       layers, batch_size,
                       Pec, Rey):
        ...
        # placeholders
        # <-- MODIFIED: Added placeholders for u, v, w, p (9 total data placeholders)
        ...
        # loss
        # <-- MODIFIED: Complete loss function overhaul
        ...
        # Combine data losses (low weight for 'c' as requested)
        # You can tune these weights further
    ...
    def train(self, total_time, learning_rate, lambda_physics=0.5): # <-- MODIFIED: Added lambda
        ...
        while running_time < total_time:
            ...
            # <-- MODIFIED: Added u, v, w, p data batches
            (t_data_batch,
             x_data_batch,
             y_data_batch,
             z_data_batch,
            ...
            # <-- MODIFIED: Added new placeholders to tf_dict
            tf_dict = {self.t_data_tf: t_data_batch,
                       self.x_data_tf: x_data_batch,
                       self.y_data_tf: y_data_batch,
                       self.z_data_tf: z_data_batch,
                       ...
                       self.z_eqns_tf: z_eqns_batch,
                       self.learning_rate: learning_rate,
                       self.lambda_physics_tf: lambda_physics} # <-- MODIFIED: Pass lambda
...
if __name__ == "__main__": 
    ...
    # <-- MODIFIED: Sample u, v, w, p data just like c_data
    ...
    # Training
    # <-- MODIFIED: Pass new data to the constructor
    model = HFM(t_data, x_data, y_data, z_data, c_data,
                u_data, v_data, w_data, p_data,
                t_eqns, x_eqns, y_eqns, z_eqns,
                layers, batch_size,
    ...
    # <-- MODIFIED: Pass the lambda value to train()
    # This is the value Member C will change for Experiment 1
    # 0.5 means 50% data loss, 50% physics loss
    # 0.9 means 10% data loss, 90% physics loss
    model.train(total_time = 0.55, learning_rate=1e-3, lambda_physics=0.5)
    ...
    print('Error c: %e' % (error_c))
    print('Error u: %e' % (error_u))  # <-- FIXED SYNTAX
    print('Error v: %e' % (error_v))
    print('Error w: %e' % (error_w))
    print('Error p: %e' % (error_p))
    ...
    # Create the grid one time, outside the loop
    # We assume x_star, y_star, z_star are the N unique spatial points
    points = np.hstack((x_star, y_star, z_star))
    grid = pv.PolyData(points) # Use PolyData, it's more robust
    ...
    for snap in range(0,t_star.shape[0]):
        ...
        # Add data to the grid for this snapshot
        # Note: We are overwriting the data in 'grid' each time
        # To save all snapshots, you would need a different logic,
        # but for saving one-by-one, this is fine.
        grid.point_data["C_test"] = c_test
        grid.point_data["U_test"] = u_test
        grid.point_data["V_test"] = v_test
        grid.point_data["W_test"] = w_test
        ...
        # Add error fields for visualization
        ...
        # Save the grid to a VTK file (using .vtp for PolyData)
        # Make sure the ../Results/Aneurysm3D/checkvtktwo/ directory exists
        grid.save("../Results/Aneurysm3D/checkvtktwo/allresult_%s.vtp" %snap) 
