...
- Experiment 4, Run 1: Baseline
- Weights: Data=1.0, BC=1.0, Physics=1.0
"""
...
class HFM(object):
    ...
    def train(self, total_time, learning_rate, 
        ...
        # Adjust batch size for BC if N_bc is smaller
        bc_batch_size = min(self.batch_size, self.N_bc)
        
        while running_time < total_time:
            
            idx_data = np.random.choice(self.N_data, self.batch_size, replace=True) # Add replace=True
            idx_bc = np.random.choice(self.N_bc, bc_batch_size, replace=True) # Add replace=True
            idx_eqns = np.random.choice(self.N_eqns, self.batch_size, replace=True) # Add replace=True
...
if __name__ == "__main__": 
    ...
    config = {
        'name': 'Exp4_Run1_Baseline_Data_BC_Phys',
        'layers': [4] + 10*[5*50] + [5],
        'total_time': 1.0, 
        'learning_rate': 1e-3, 
        'batch_size': 10000,
