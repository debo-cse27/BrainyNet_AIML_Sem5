"""
Experiment 5: Activation Variation
Variations: tanh, sigmoid, relu, swish
...
from utilities import Navier_Stokes_3D, tf_session, mean_squared_error, relative_error

# Custom Neural Net Builder to support Activation swapping
def custom_neural_net(t, x, y, z, layers, activation_name):
    # Determine activation and initializer
    if activation_name == 'relu':
        act_func = tf.nn.relu
        # He Normal
        init_func = tf.keras.initializers.VarianceScaling(scale=2.0, mode='fan_in', distribution='normal')
    elif activation_name == 'tanh':
        act_func = tf.tanh
        init_func = tf.glorot_normal_initializer() # Xavier
    elif activation_name == 'sigmoid':
        act_func = tf.sigmoid
        init_func = tf.glorot_normal_initializer() # Xavier
    elif activation_name == 'swish':
        act_func = tf.nn.swish
        init_func = tf.keras.initializers.VarianceScaling(scale=2.0, mode='fan_in', distribution='normal') # Often uses He
    else:
        raise ValueError("Unknown activation")

    X = tf.concat([t, x, y, z], 1)
    ...
    with tf.variable_scope("neural_net", reuse=tf.AUTO_REUSE):
        ...
        for l in range(0, num_layers-2):
            W = tf.get_variable(f"W{l}", shape=[layers[l], layers[l+1]], initializer=init_func)
            b = tf.get_variable(f"b{l}", shape=[1, layers[l+1]], initializer=tf.zeros_initializer())
            X = act_func(tf.add(tf.matmul(X, W), b))
            
        W = tf.get_variable(f"W{num_layers-2}", shape=[layers[-2], layers[-1]], initializer=init_func)
        b = tf.get_variable(f"b{num_layers-2}", shape=[1, layers[-1]], initializer=tf.zeros_initializer())
...
class HFM(object):
    def __init__(self, t_data, x_data, y_data, z_data, c_data,
        ...
        [self.c_data_pred, self.u_data_pred, self.v_data_pred, self.w_data_pred, self.p_data_pred] = \
            custom_neural_net(self.t_data_tf, self.x_data_tf, self.y_data_tf, self.z_data_tf, self.layers, self.activation)
            
        [self.c_eqns_pred, self.u_eqns_pred, self.v_eqns_pred, self.w_eqns_pred, self.p_eqns_pred] = \
            custom_neural_net(self.t_eqns_tf, self.x_eqns_tf, self.y_eqns_tf, self.z_eqns_tf, self.layers, self.activation)
        ...
        [self.e1_eqns_pred, self.e2_eqns_pred, self.e3_eqns_pred, self.e4_eqns_pred, self.e5_eqns_pred] = \
            Navier_Stokes_3D(self.c_eqns_pred, self.u_eqns_pred, self.v_eqns_pred, self.w_eqns_pred, self.p_eqns_pred,
                             self.t_eqns_tf, self.x_eqns_tf, self.y_eqns_tf, self.z_eqns_tf, self.Pec, self.Rey)
...
if __name__ == "__main__": 
    results_dir = "../Results/Experiment5_Activation"
    ...
    for act in activations:
        ...
        vtk_dir = f"{results_dir}/vtk_act_{act}"
        ...
        scipy.io.savemat(f"{results_dir}/results_act_{act}.mat",
