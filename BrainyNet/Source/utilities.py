...
def tf_session():
    ...
    config = tf.ConfigProto(allow_soft_placement=True,
                            log_device_placement=False)  # Set to False to reduce verbosity
    # Note: force_gpu_compatible is deprecated, but kept for compatibility
    try:
...
def radon_projection(c, t, x, y, z, T):
    c_x = fwd_gradients(c, x)
    c_y = fwd_gradients(c, y)
    c_z = fwd_gradients(c, z)
    nx,ny,nz = -math.cos(t*T/2.0*math.pi), -math.sin(t*T/2.0*math.pi),0.0
    proj_vec = nx*c_x + ny*c_y + nz*c_z
    return proj_vec
