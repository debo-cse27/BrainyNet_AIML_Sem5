"""
Experiment 7: Data Density Variation
Variations: 5000, 10000, 20000, 40000, 98700 points
...
class HFM(object):
    ...
    def train(self, total_time, learning_rate):
        ...
        # Handle batch size smaller than N_data if density is very low
        real_batch_size = min(self.batch_size, N_data)
        
        print('It, Loss, Time(s), Running Time(h), Learning Rate')
        while running_time < total_time:
            
            idx_data = np.random.choice(N_data, real_batch_size)
            idx_eqns = np.random.choice(N_eqns, real_batch_size)
...
if __name__ == "__main__": 
    results_dir = "../Results/Experiment7_DataSize"
    if not os.path.exists(results_dir): os.makedirs(results_dir)
    
    data_sizes = [5000, 10000, 20000, 40000, 98700]
    ...
    C_star = data['C_star']; U_star = data['U_star']; V_star = data['V_star']; W_star = data['W_star']; P_star = data['P_star']
    
    T_star_grid = np.tile(t_star, (1,N)).T
    X_star_grid = np.tile(x_star, (1,T))
    Y_star_grid = np.tile(y_star, (1,T))
    Z_star_grid = np.tile(z_star, (1,T))
    
    layers = [4] + 10*[5*50] + [5]
    
    for size in data_sizes:
        print(f"\n======== STARTING RUN: Data Size = {size} ========\n")
        tf.reset_default_graph()
        
        # Prepare Data Slice based on size
        total_points_avail = T * N
        idx_flat = np.random.choice(total_points_avail, size, replace=False)
        
        # Flatten everything
        t_data_full = T_star_grid.flatten()[:,None]
        x_data_full = X_star_grid.flatten()[:,None]
        y_data_full = Y_star_grid.flatten()[:,None]
        z_data_full = Z_star_grid.flatten()[:,None]
        c_data_full = C_star.flatten()[:,None]
        
        # Subset
        t_data = t_data_full[idx_flat]
        x_data = x_data_full[idx_flat]
        y_data = y_data_full[idx_flat]
        z_data = z_data_full[idx_flat]
        c_data = c_data_full[idx_flat]
        
        # Equation points usually stay dense or follow data? 
        # Standard approach: use same distribution or specialized sampling.
        # Here we will use the same random subset logic for simplicity or keep full.
        # Keeping full for equations might be too heavy if size is small, but standard is N_data points.
        # Let's match equation points to data points count for fairness in "density" variation.
        t_eqns = t_data
        x_eqns = x_data
        y_eqns = y_data
        z_eqns = z_data
        ...
        model.train(total_time=2.0, learning_rate=1e-3)
        
        snap = 10
        t_test = T_star_grid[:,snap:snap+1]; x_test = X_star_grid[:,snap:snap+1]
        y_test = Y_star_grid[:,snap:snap+1]; z_test = Z_star_grid[:,snap:snap+1]
        
        c_pred, u_pred, v_pred, w_pred, p_pred = model.predict(t_test, x_test, y_test, z_test)
        
        error_c = relative_error(c_pred, C_star[:,snap:snap+1])
        error_u = relative_error(u_pred, U_star[:,snap:snap+1])
        error_v = relative_error(v_pred, V_star[:,snap:snap+1])
        error_w = relative_error(w_pred, W_star[:,snap:snap+1])
        error_p = relative_error(p_pred - np.mean(p_pred), P_star[:,snap:snap+1] - np.mean(P_star[:,snap:snap+1]))
        ...
        vtk_dir = f"{results_dir}/vtk_size_{size}"
        if not os.path.exists(vtk_dir): os.makedirs(vtk_dir)

        for snap_idx in range(0, data['t_star'].shape[0]):
            t_test = T_star_grid[:,snap_idx:snap_idx+1]
            x_test = X_star_grid[:,snap_idx:snap_idx+1]
            y_test = Y_star_grid[:,snap_idx:snap_idx+1]
            z_test = Z_star_grid[:,snap_idx:snap_idx+1]
        ...
        scipy.io.savemat(f"{results_dir}/results_size_{size}.mat",
